...
class ParsingSystem:
    ...
    def apply(self, configuration: Configuration, transition: str) -> Configuration:
        ...
        """
        # TODO(Students) Start
        w1 = configuration.get_stack(0)
        w2 = configuration.get_stack(1)
        label = transition[transition.find("(")+1:transition.find(")")]


        if transition.startswith('L'):
            configuration.remove_second_top_stack()
            configuration.add_arc(w1, w2, label)
            
        elif transition.startswith('R'):
            configuration.remove_top_stack()
            configuration.add_arc(w2, w1, label)
