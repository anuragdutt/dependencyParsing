    ...
    # for i in tqdm(range(50)):
        if trees[i].is_projective():
            ...
            while not parsing_system.is_terminal(c):
                ...
                # print(parsing_system.is_terminal(c))
    return instances
...
def get_configuration_features(configuration: Configuration,
    ...
    for i in range(3):
        stack.append(configuration.get_stack(i))
    for i in range(3):
        buff.append(configuration.get_buffer(i))
    ...
    for i in range(2):
        lc1 = configuration.get_left_child(stack[i], 1)
        rc1 = configuration.get_right_child(stack[i], 1)
        lc2 = configuration.get_left_child(stack[i], 2)
        rc2 = configuration.get_right_child(stack[i], 2)

        lst = [lc1, rc1, lc2, rc2]
        lcrc.extend(lst)

        llc1 = configuration.get_left_child(lc2, 1)
        rrc1 = configuration.get_right_child(rc2, 1)

        llcrrc.extend([llc1, rrc1])
    ...
    lbl_lst = []
    lbl_lst.extend(lcrc)
    lbl_lst.extend(llcrrc)

    features = []


    st = [configuration.get_pos(w) for w in sw]
    sl = [configuration.tree.get_label(w) for w in lbl_lst]

    sw_id = [vocabulary.get_word_id(w) for w in sw]
    st_id = [vocabulary.get_pos_id(w) for w in st]
    sl_id = [vocabulary.get_label_id(w) for w in sl]

    features.extend(sw_id)
    features.extend(st_id)
    features.extend(sl_id)
